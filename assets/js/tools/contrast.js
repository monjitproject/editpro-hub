/**
 * Contrast Tool
 * Adjusts the contrast of an image by scaling pixel values around midpoint.
 * Controls: range#contrast (-100 to 100, default 0)
 */
document.addEventListener('DOMContentLoaded', () => {
  ...
  function processImage() {
    ...
    if (contrast !== 0) {
      ...
      Utils.adjustContrast(imageData, contrast);
  ...
  if (contrastSlider) {
    contrastSlider.addEventListener('input', debouncedProcess);
  }
  ...
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      ...
      processImage();
      Utils.showToast('Contrast reset to default', 'info');
    });
  }
